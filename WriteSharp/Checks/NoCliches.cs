...
internal class NoCliches : IChecker
{
    ...
    public NoCliches()
    {
        ...
        {
            cliches[i] = cliches[i].Replace(" ", "[\\b\\s\\u200C]*");
        }

        string clichesString = string.Join("|", cliches);
        _regex = new Regex($"\\b({clichesString})\\b", RegexOptions.IgnoreCase);
    }
    ...
    public List<CheckResult> Check(string text)
    {
        ...
        foreach (Match match in _regex.Matches(text))
        {
            CheckResult result = new CheckResult()
            {
                Index = match.Index,
                Offset = match.Length,
                Reason = $"\"{match.Value}\" is a cliche"
            };
            results.Add(result);
        }
